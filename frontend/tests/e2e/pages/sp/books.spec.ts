...
test.describe('Books', () => {
  ...
  const waitAccordionOpen = async page => {
    ...
    await waitAnimation(
      page,
      FilterAccordionContentSelector,
      'maxHeight',
      `${String(Number(vh.replace('px', '')) * 100 - 120 - 40)}px`
    );
    await page.waitForSelector('.spinner-border', { state: 'hidden' });
  ...
  test('04_activate_genre_selector', async ({ page }) => {
    ...
    const expectedColor = hexToRgb(
      await getStyleFromRoot(page, '--color-text-selection')
    );
    ...
    await page.waitForFunction(
      ([selector, expectedColor]) => {
        ...
        return (
          window.getComputedStyle(element).backgroundColor === expectedColor
        );
      },
      [selector, expectedColor]
    );
  ...
  test('07_show_float_filter', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const el = document.documentElement;
      const bottom = el.scrollHeight - el.clientHeight - 10;
      window.scroll(0, bottom);
    });
    await page.waitForFunction(() => {
      const el = document.documentElement;
    ...
    await page.waitForFunction(
      ({ selector, expect }) => {
        const el = document.querySelector(selector);
        return (
          el.getBoundingClientRect().top === Number(expect.replace('px', ''))
        );
      },
      { selector: floatAccordionSelector, expect }
    );

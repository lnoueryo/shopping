...
const waitInitialDisplay = async page => {
  await page.goto(
    '/books?keyword=ノーコードでつくるWebサイト　ツール選定・デザイン・制作・運用が全部わかる！'
  );
...
test.describe('Books', () => {
  ...
  test('01_hover_ratings', async ({ page }) => {
    ...
    const ratingFourSelector =
      '#main-content .bookfilter-container fieldset.rate-form label:nth-child(3)';
    await page.locator(ratingFourSelector).hover();
    const expectedColor = hexToRgb(
      await getStyleFromRoot(page, '--color-hover-rating', ratingFourSelector)
    );
  ...
  test('02_activate_ratings', async ({ page }) => {
    ...
    await waitAnimation(page, ratingFourSelector, 'color', expectedColor);
    await page.waitForSelector('.spinner-border', { state: 'hidden' });
    await page.waitForSelector('#error-book-result', { state: 'visible' });
    await page.waitForSelector('#server-error', { state: 'hidden' });
    await page.waitForSelector('.linear', { state: 'hidden' });
    await page.screenshot({
      path: `${BASE_IMAGE_PATH}/${browserName}/${fileName}`,
    });
  });
  ...
  test('03_hover_skill_levels', async ({ page }) => {
    await waitInitialDisplay(page);
    const skillLevelFirstSelector =
      '#main-content .bookfilter-container fieldset:not(.rate-form) .next-space label';
    await page
      .locator(skillLevelFirstSelector)
      .filter({ hasText: 'Beginner' })
      .hover();
    const expectedOpacity = await getStyleFromRoot(
      page,
      '--opacity-hover',
      skillLevelFirstSelector
    );
    if (browserName === 'firefox') {
      forceChangeStyle(
        page,
  ...
  test('04_activate_skill_levels', async ({ page }) => {
    ...
    for (let i = 0; i < skillLevelsElements.length; i++) {
      ...
      const expectedColorVar = hexToRgb(
        await getStyleFromRoot(page, skillLevelsData[i].color)
      ); // CSS変数名
      const title =
        skillLevelsData[i].title.charAt(0).toUpperCase() +
        skillLevelsData[i].title.slice(1);

      await skillLevel.click();
      await waitAnimationByText(
        page,
        skillLevelFirstSelector,
        title,
        'backgroundColor',

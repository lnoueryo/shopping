...
test.describe('Books', () => {
  ...
  test.describe('initial screen', () => {
    ...
    const ENDPOINT = '/books?keyword=良いコード／悪いコードで学ぶ設計入門&rate=4&levels=beginner';
    const THRESHOLD = 10;
    ...
    test('Verify initial display on PC', async ({ page }) => {
      ...
      if (numDiffPixels > THRESHOLD) createDiffImage(project, diff, screenshotBuffer);
    ...
    test('Verify initial display on android', async ({ browser }) => {
      ...
      if (numDiffPixels > THRESHOLD) createDiffImage(android, diff, screenshotBuffer);
    ...
    test('Verify initial display on iphone', async ({ browser }) => {
      ...
      if (numDiffPixels > THRESHOLD) createDiffImage(iphone, diff, screenshotBuffer);

<script setup lang="ts">
  ...
  watch(
    [localRate, localSkillLevels],
    async () => {
      await searchBooks.filterBooks(localRate.value, localSkillLevels.value);
    },
    { deep: true }
  );
  watch(
    ...
    newValue => {
      ...
      const newLevels = Array.isArray(levels)
        ? levels
        : !levels
          ? []
          : [levels];
      if (
        JSON.stringify(newLevels) !== JSON.stringify(localSkillLevels.value)
      ) {

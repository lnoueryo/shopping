...
#### 404
- **エラーページ**
  -**Purpose**: デフォルトの404をサイトオリジナルにすることと、SPAでの404はサーバーを返さないためまた別途必要となる
  - **Acceptance Criteria**:
    - URLから直接404ページにアクセスした場合404ページを返す
    - ヘッダーを表示し、ホームにリダイレクトするボタンを設置
    - 使用中に何かしらの404ページに移動した場合、ヘッダーと前のページに戻るボタンを設置
  ---
#### Connection-error
  -**Purpose**: 通信が不安定でデータを取得できていないことを知ることができる
  - **Acceptance Criteria**:
    - 通信状態が悪くタイムアウトした場合に/errorページに遷移する
    - ステータスコードと前回のパスをパラメーターにのせる: `/error?next=/frontend&status=503`
    - ヘッダーを表示し、前回のページにリダイレクトするボタンを設置
    ※リロードした場合errorのページが表示されているわけではなくfrontend/error.vueが標示されている
#### ETC
- **オフライン通知**:
...
- **ファイルのキャッシュ**:
  ...
  - **Acceptance Criteria**:
    - サービスワーカーでキャッシュするファイルを登録する
    - キャッシュからファイルを取得し、表示する
